...
/**
 * 07 · ESITI — il tempo invece dello slider.
 *
 * Il ritratto resta fermo; a scoprirlo è una linea che scende, la
 * stessa che disegna il profilo nella soglia e sottolinea i campi
 * del modulo. Il confronto è un attraversamento, non un cursore da
 * trascinare avanti e indietro.
 */
export function initEsiti() {
  const scene = document.querySelector("#esiti");
  if (!scene) return;

  const lastra = scene.querySelector("[data-esiti-lastra]");
  const marca = scene.querySelector("[data-esiti-marca]");
  const tappe = [...scene.querySelectorAll("[data-esiti-tempo] li")];
  if (!lastra) return;

  const ETICHETTE = [
    { fino: 0.34, testo: "T0" },
    { fino: 0.68, testo: "T+14 giorni" },
    { fino: 1.01, testo: "T+90 giorni" },
  ];

  const applica = (p) => {
    lastra.style.setProperty("--rivela", `${p * 100}%`);
    lastra.style.setProperty("--rivela-op", p > 0.01 && p < 0.99 ? "1" : "0");
    const i = ETICHETTE.findIndex((e) => p < e.fino);
    const idx = i === -1 ? ETICHETTE.length - 1 : i;
    if (marca && marca.textContent !== ETICHETTE[idx].testo) {
      marca.textContent = ETICHETTE[idx].testo;
    }
    tappe.forEach((t, n) => t.toggleAttribute("data-attivo", n === idx));
  };

  if (reducedMotion) {
    applica(1);
    tappe.forEach((t) => t.setAttribute("data-attivo", ""));
    return;
  }
  ...
  gsap.to(stato, {
    ...
    onUpdate: () => applica(stato.p),
    scrollTrigger: {
      trigger: scene.querySelector("[data-esiti-tempo]"),
      start: "top 72%",
      end: "bottom 72%",
      scrub: 0.9,
    },
